...
--
-- Host: localhost
-- Generation Time: Feb 16, 2020 at 05:24 AM
...
SET stmt = concat(stmt, ' where s.bclosed = 1 ');
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `APP_UPDATE_HISTORY_REQUEST` (IN `sreview` VARCHAR(50) CHARSET utf8, IN `nrating` BIGINT(20), IN `nid` BIGINT(20) UNSIGNED)  BEGIN
UPDATE `history_request_list`
SET `nrating`= `nrating`,
`sreview`=`sreview`
WHERE `id`=`nid`;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `APP_UPDATE_REQUEST` (IN `scause` VARCHAR(50) CHARSET utf8, IN `nprice` DECIMAL(17,0), IN `bclosed` BOOLEAN, IN `nstatus` BIGINT(20), IN `nuser` BIGINT(20), IN `nid` BIGINT(20) UNSIGNED)  BEGIN

UPDATE `request_list`
SET `nstatus`= `nstatus`,
`nuser`=`nuser`, `nprice`=`nprice`, `scause`=`scause`, `bclosed` = `bclosed`
WHERE `id`=`nid`;
...
CREATE TABLE `history_request_list` (
  ...
  `slanguage` char(5) COLLATE utf8_unicode_ci NOT NULL DEFAULT 'uk_UA'
...
CREATE TABLE `next_statuses` (
  `nstatus` bigint(20) NOT NULL,
  `nnext_status` bigint(20) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
ALTER TABLE `next_statuses`
  ADD KEY `FKsuta6axf3migey5h9oj16xj5x` (`nnext_status`),
...
ALTER TABLE `next_statuses`
  ...
  ADD CONSTRAINT `FKsuta6axf3migey5h9oj16xj5x` FOREIGN KEY (`nnext_status`) REFERENCES `status` (`id`);
